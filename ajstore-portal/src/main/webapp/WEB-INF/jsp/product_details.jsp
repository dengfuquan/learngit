...
<body>
	...
	<div id="shop_detail">
		...
		<div id="preview" class="lf">
			<div id="mediumDiv">
				<img id="mImg" src="${list.get(1).params.get(0).form}" />
			</div>
			<div id="icon_all">
				<ul id="icon_list">
					<!-- var 变量名不能是jstl的关键字 -->
					<c:forEach items="${list.get(1).params}" var="itemParam">
						<li class="i1"><img src="${itemParam.key}" /></li>
					</c:forEach>
				</ul>
			</div>
		</div>
		...
		<div class="right_detail lf">
			<!-- 商品名称-->
			<h1>${ajiaItem.brand}&nbsp;&nbsp;${ajiaItem.model}</h1>
			<!-- 商品全称-->
			<h3>${ajiaItem.title}</h3>
			<!-- 价格部分-->
			<div class="price">
				<div id="pro_price">
					<b>学员售价：</b><span>￥${ajiaItem.price}</span>
				</div>
				<div class="promise">
			...
			<p class="style" id="choose_color">
				<s class="color">颜色：</s>
				<c:forEach items="${typeParamData.params}" var="paramData">
					<c:if test="${paramData.key=='颜色' }">
						<c:if test="${paramData.values!=null}">
							<c:forEach items="${paramData.values}" var="showColor">
								<!-- 高亮显示当前商品的颜色 -->
								<c:forEach items="${list.get(0).params}" var="itemParamData">
									<c:if test="${itemParamData.key=='颜色'}">
										<c:if test="${itemParamData.values!=null }">
											<c:forEach items="${itemParamData.values}" var="itemColor">
												<input type="button"
													class="i1 ${showColor==itemColor?'borderChange':''}"
													value="${showColor}" title="${color}" />
											</c:forEach>
										</c:if>
									</c:if>
								</c:forEach>
			...
			<p>
				<s>规格：</s>
				<c:forEach items="${typeParamData.params}" var="paramData">
					<c:if test="${paramData.key=='型号'}">
						<c:if test="${paramData.values!=null}">
							<c:forEach items="${paramData.values}" var="showModel">
								<c:forEach items="${list.get(0).params}" var="itemParamData">
									<c:if test="${itemParamData.key=='型号' }">
										<c:if test="${itemParamData.values!=null}">
											<c:forEach items="${itemParamData.values}" var="itemModel">
												<span
													class="avenge ${showModel==itemModel?'borderChange':'' }">${showModel}</span>
											</c:forEach>
										</c:if>
									</c:if>
								</c:forEach>
			...
			<div class="all">
				...
				<div id="sale_protection">
					...
					<div class="sale_content">
						...
						<p class="content">阿甲学子商城向您保证所售商品均为正品行货，阿甲自营商品开具机打发票或电子发票。</p>
	...
	<script>
		$(function() {
				...
				$(this).click(function(norms) {
					...
					$.ajax({
						...
						success : function(response) {
							...
							//js重定向，告诉浏览器加载指定的网页
							location.href = "/toItemInfo/" + itemId + ".html";
						},
						error : function(e) {
							console.log("失败");
						}
			...
			//颜色选择
			$("#choose_color input").each(
					function(i, item) {
						$(this).click(
								function() {
									$(this).addClass("borderChange")
									if ($(this).siblings().addClass(
											"borderChange")) {
