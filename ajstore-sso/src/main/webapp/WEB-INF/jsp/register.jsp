...
<div class="parent">
  ...
  <div class="container">
    <div class="panel rt">
      <form id="form-register" method="post" action="/user/register.html">
        <div class="txt">
          <p>新用户注册
            <span>
        ...
        <div class="form-group">
          ...
          <input autocomplete required minlength="6" maxlength="9" type="text" placeholder="请输入用户名" autofocus name="username" id="uname" value="123451"/>
          <span class="msg-default">用户名长度在6到9位之间</span>
        </div>
        <div class="form-group">
          <label for="upwd">登录密码：</label>
          <input value="111111" required type="password" minlength="6" maxlength="12" placeholder="请输入密码" name="password" autofocus id="upwd"/>
          <span class="msg-default hidden">密码长度在6到12位之间</span>
        </div>
		 <div class="form-group">
          <label for="upwdconfirm">确认密码：</label>
          <input value="111111" required type="password" minlength="6" maxlength="12" placeholder="请确认密码" name="upwdconfirm" autofocus id="upwdconfirm"/>
          <span class="msg-default hidden">密码长度在6到12位之间</span>
        </div>
        <div class="form-group">
          <label for="email">邮箱：</label>
          <input value="1@ajia.com" autocomplete required type="email" placeholder="请输入邮箱地址" name="email" id="email"/>
          <span class="msg-default hidden">请输入合法的邮箱地址</span>
        </div>
        <div class="form-group">
          <label for="phone">手机号：</label>
          <input value="13691481011" id="phone" name="phone" placeholder="请输入您的手机号" pattern="(\d{11})|^((\d{7,8})|(\d{4}|\d{3})-(\d{7,8})|(\d{4}|\d{3})-(\d{7,8})-(\d{4}|\d{3}|\d{2}|\d{1})|(\d{7,8})-(\d{4}|\d{3}|\d{2}|\d{1}))$" required type="text" />
...
<script>
  ...
  uname.onblur = function(){
    if(this.validity.valueMissing){
      ...
      //3 打开连接
      xhr.open('GET','/user/checkUser.html?username='+data, true);
      //4 发送请求
      xhr.send(null);
      ...
      function doResponse(xhr){
        ...
        console.log(object);
        if(object.data==='用户名已经存在'){
          var $uname=$("#uname");
          var $span=$uname.siblings(".msg-success");
          console.log($span);
          $span.html('用户名已经存在');
        }else if(object.data==='用户名不存在'){
        	 var $uname=$("#uname");
             var $span=$uname.siblings(".msg-success");
             console.log($span);
             $span.html('用户名不存在');
